package object similarity {
  ...
  type Candidates = Array[Similarity]
...
package similarity {
  ...
  object utils {
    ...
    def stringList_to_hash(lst:List[String]):Int = {
      crc32(lst.mkString(SEP))
    }

    def intList_to_hash(lst:List[Int]) = {
      crc32(lst.toString)
    }
    ...
    def do_report(jaccardResults: Matches, minHashCandidates: Candidates, lshCandidates: Candidates) = {
      ...
      def results_to_map (recs: Array[Similarity]):Map[String, String] = {
        recs.map(r => (r.idA + "," + r.idB, double_to_string(r.sim))).toMap.withDefaultValue("---")
      }

      val jac = results_to_map(jaccardResults)

      val minhash = results_to_map(minHashCandidates)
      val lsh = results_to_map(lshCandidates)
